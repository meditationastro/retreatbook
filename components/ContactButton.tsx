...
import { MessageCircle,MessageCircleIcon, Facebook, Instagram, Youtube, Mail, Phone, X } from 'lucide-react';

// Inline WhatsApp SVG icon as a React component
// const WhatsAppIcon = (props: React.SVGProps<SVGSVGElement>) => (
//   <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 50 50" scale={50}>
...
export const ContactButton = () => {
  ...
  const socialLinks = [
    { icon: MessageCircleIcon, href: 'https://wa.me/9779823376110', label: 'WhatsApp' },
    {icon:Phone, href: 'tel:+977 982-3376110', label: 'Call Us'},
    { icon: Facebook, href: 'https://www.facebook.com/meditationastro/', label: 'Facebook' },
    { icon: Instagram, href: 'https://www.instagram.com/meditationastro_1/', label: 'Instagram' },
    { icon: Youtube, href: 'https://www.youtube.com/@astromeditation1', label: 'YouTube' },
    { icon: Mail, href: 'mailto:meditationastro1@gmail.com', label: 'Contact' },
  ];
    ...
    >
      <div className="relative">
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              className="absolute bottom-16 right-0 flex flex-col justify-center items-center gap-2 w-14 "
            ...
            >
              {socialLinks.map((social, index) => (
                <motion.a
                  key={social.label}
                  href={social.href}
                  className="flex items-center justify-center w-10 h-10 bg-primary-100 hover:bg-primary/20 text-primary rounded-full transition-colors"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 20 }}
                  transition={{ 
        ...
        <motion.button
          onClick={toggleOpen}
          className="flex items-center justify-center w-14 h-14 bg-blue-800 text-white hover:bg-primary/90 rounded-full shadow-lg"
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          {isOpen ? <X className="w-6 h-6 trans" /> : <MessageCircle className="w-6 h-6" />}
        </motion.button>
      </div>
    </motion.div>
  );
